...
date: 2022-8-11
tags: ['flutter', 'flutter-web', 'responsive-serie']
draft: false
...
La forma como yo lo hago es, tener puntos de control, voy a listar algunas ideas para estos puntos de control:
...
En mi opinión es una herramienta útil como lo es MediaQuery para poder hacer muchas cosas con tamaños locales, orientación y poder renderizar diferentes widgets depende de las situación.
...
2. Puedes usar este [repositorio][] la rama (**dashboard-web-responsive**), con el panel listo en web y saltar hasta la sección, **Empecemos el responsive**.
...
Lo primero que voy a hacer es crear una widget que me ayude a saber los puntos de control. Este widget será basado en **LayoutBuilder** y dependiendo del tamaño de la pantall vamos a devolver una pantalla especifica para **mobile**, **tablet** o **web**.
...
El siguiente paso es cambiar el home_page.dart que teniamos antes por:
...
El web_layout.dart va quedar como era el antiguo **HomePage**, los otros dos MobileLayout y TabletLayout por el momento vamos dejarlos vacios.
...
Hasta no hemos hecho mucho pero veamos que pasa si movemos el tamaño de la pantalla. Notaran que tenemos problemas de _overflow_ en el WebLayout.
...
En el web*layout.dart y coloquemos un **LayoutBuilder** y vamos a ocultar el *NavMenu* y el \_SideBar* vamos a mostar un butón con icon cuando se oculte el menu.
...
⚠️ Aun queda solucionar algunos overflow en el cuando el **height** cambia.
